...
> 作者：ConardLi（花园老师） | 来源：[code秘密花园](https://mp.weixin.qq.com/)（公众号）
>
> 整理日期：2026-08-04
...
这篇文章把 AI 工程领域的三次重心迁移讲得很清楚：Prompt Engineering → Context Engineering → Harness Engineering。重点落在 Harness 上——当一个 Agent 系统除了模型本身之外的所有部分（上下文管理、工具系统、执行编排、状态与记忆、评估与观测、约束校验与失败恢复），并拆解了 Anthropic、OpenAI、LangChain 的真实工程实践。和本 workspace 的核心议题（context infrastructure）高度相关，可作为方法论的参照系。
...
- **约束**：限制模型可做和不可做的事，比如：哪些工具能用、哪些场景必须查证、哪些内容涉及安全边界
- **校验**：在输出前做检查，比如：是否回答了用户问题、是否遗漏关键要求、是否满足格式规范
...
第一个是**上下文焦虑**：任务一长，上下文窗口越来越满，模型开始丢细节、丢重点，甚至出现一种很有意思的现象：当模型接近上下文窗口极限时，它会 "焦虑" 地想赶紧收尾，仿佛知道自己快装不下了。

第二个是**自评失真**：也就是模型自己做完之后，再让它自己评判，它往往会偏乐观，尤其在设计、体验、产品完整度这类没有绝对二元答案的问题上，这种偏差更明显。
...
这样一来，代码质量就不再主要靠人盯，而是靠一套能反复执行的规则来守住。对人来说，这些规则有时可能显得太细；但对 Agent 来说，它们非常重要。
...
只是越往后，我们越会意识到：真正决定 AI 产品上限的，也许是模型；但真正决定 AI 产品能否落地、能否稳定交付的，往往是 Harness。
...
如果本期教程对你有所帮助，大家多多点赞、收藏、关注支持我，后续我会分享更多我个人在 Harness Engineering 具体的工程实践。
