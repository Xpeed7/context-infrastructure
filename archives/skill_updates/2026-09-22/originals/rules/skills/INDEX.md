...
索引表示资料可查，不表示相关工具已安装或验证。运行前核对当前客户端的工具、依赖和权限；自动记忆状态见 [CRONTAB.md](../../docs/CRONTAB.md)。

独立检索或交叉验证需要子代理时，读取 [并行工作流](./workflow_parallel_subagents.md)。调用方式以当前客户端为准，不支持时串行完成。
...
- [分享报告到 Web](./share_report.md) — 将 Markdown 报告转为 HTML 发布至自建服务器并返回访问 URL
- [Apple Compressor Skill](./compressor.md) — 本机 Apple Compressor CLI 转码；custom preset 路径、源文件写入完成检测、batch 提交与监控
...
- [延时执行技能](./delayed_execution.md) — `sleep + nohup` 低风险 fallback；持久或 AI 延时任务按当前客户端能力接入调度器
...
创建或重写前，先读 [Skill 写作指南](./bestpractice_skill_writing.md)。文件按 `<category>_<name>.md` 命名，添加到对应分类。索引只保留触发场景和入口，操作细节留在技能正文。
