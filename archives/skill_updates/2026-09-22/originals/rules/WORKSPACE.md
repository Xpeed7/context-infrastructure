...
- AI 会话归档：`contexts/ai_sessions/<source>/`（约定输出目录，当前尚未创建；导出能力先核验，搜索流程见 `rules/skills/ai_session_search_archive.md`）

### 系统与规则
- 可复用技术方案 / Skill：`rules/skills/`
- 核心公理（Axioms）：`rules/axioms/`
- 记忆系统：`contexts/memory/` + `periodic_jobs/ai_heartbeat/`
- 本工程历史文档与废弃规则：`archives/`，各批次说明见 `archives/README.md`；原版按原相对路径保存在批次的 `originals/`，不默认加载或检索。
...
## Python 环境
- 根目录 `.venv/` 是工作区级环境的约定路径，当前尚未创建；运行脚本前核对实际环境，已有 uv 环境用 `uv pip install` 管理依赖
